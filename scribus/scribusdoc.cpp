...
void ScribusDoc::replaceNamedResources(ResourceCollection& newNames)
{
	...
	for (NotesStyle* nStyle : std::as_const(m_docNotesStylesList))
...
void ScribusDoc::restoreNoteStyle(SimpleState* ss, bool isUndo)
{
	...
	else if (ss->get("NSTYLE") == "edit")
	{
		...
		if (delNF)
		{
			...
			for (PageItem_NoteFrame* noteFrame : std::as_const(noteFrameList))
...
void ScribusDoc::itemSelection_LowerItem()
{
	...
	for (auto c = Oindex.count() - 1; c > -1; c--)
	{
...
void ScribusDoc::itemSelection_RaiseItem()
{
	...
	QList<int> Oindex = ObjOrder.values();
	for (qsizetype i = 0; i < Oindex.count(); ++i)
	{
		qsizetype d = itemList->indexOf(b2);
		if (d == -1)
			d = 0;
...
void ScribusDoc::itemSelection_Transform(int nrOfCopies, const QTransform& matrix, AnchorPoint basepoint)
{
	int docSelectionCount = m_Selection->count();
	if (docSelectionCount == 0)
		return;
...
//			currItem->FrameType = 3;
	...
	else
	{
		...
		AnchorPoint rotBack = rotationMode();
		setRotationMode ( AnchorPoint::TopLeft );
		ScriXmlDoc xmlDoc;
...
void ScribusDoc::rotateItem(double angle, PageItem *currItem)
{
	...
	if (m_rotMode != AnchorPoint::TopLeft)
	{
		QTransform ma;
		ma.translate(currItem->xPos(), currItem->yPos());
		ma.scale(1, 1);
...
bool ScribusDoc::sizeItem(double newW, double newH, PageItem *pi, bool fromMP, bool DoUpdateClip, bool redraw)
{
	...
	*/
	...
	if ((m_rotMode != AnchorPoint::TopLeft) && fromMP && (!isLoading()) && (appMode == modeNormal))
	{
		QTransform ma;
		ma.rotate(currItem->rotation());
...
void ScribusDoc::scaleGroup(double scx, double scy, bool scaleText, Selection* customSelection, bool scaleLine)
{
	...
	itemSelection->getGroupRect(&gx, &gy, &gw, &gh);
	m_rotMode = drm;
	if ((m_rotMode != AnchorPoint::TopLeft) && (!isLoading()))
	{
		switch (m_rotMode)
		{
			case AnchorPoint::None:
...
void ScribusDoc::itemSelection_UniteItems(Selection* /*customSelection*/)
{
	...
	AnchorPoint oldRotMode = m_rotMode;
	m_rotMode = AnchorPoint::TopLeft;
	adjustItemSize(currItem);
...
void ScribusDoc::setRotationMode(AnchorPoint val)
{
	if (m_rotMode == val)
		return;
	m_rotMode = val;
...
void ScribusDoc::updateNumbers(bool updateNumerations)
{
	...
	for (NumStruct * numS : std::as_const(numerationValues))
...
void ScribusDoc::deleteNotesStyle(const QString& nsName)
{
	...
	for (TextNote* note : std::as_const(m_docNotesList))
	{
...
{
	bool docWasChanged = false;
	for (NotesStyle* ns : std::as_const(m_docNotesStylesList))
	{
		if (ns->isEndNotes())
		{
...
void ScribusDoc::updateNotesFramesSettings(const NotesStyle *noteStyle)
{
	...
	for (PageItem_NoteFrame* nF : std::as_const(noteFramesList))
	{
...
void ScribusDoc::updateEndnotesFrames(const NotesStyle* nStyle, bool invalidate)
{
	...
	else if (nStyle->isEndNotes())
	{
		...
		for (PageItem_NoteFrame* nF : std::as_const(noteFrameList))
...
void ScribusDoc::updateEndNotesFrameContent(PageItem_NoteFrame *nF, bool invalidate)
{
	...
	else
	{
		...
		for (const NotesStyle* noteStyle : std::as_const(m_docNotesStylesList))
		{
...
bool ScribusDoc::validateNSet(const NotesStyle& noteStyle, QString newName)
{
	...
	QString errStr;
	for (const NotesStyle* NS2 : std::as_const(m_docNotesStylesList))
	{
		if (newName.isEmpty())
